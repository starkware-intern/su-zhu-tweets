...
@pytest.fixture(scope='module')
def contract_defs():
    account_def = get_contract_def('libs/cairo-contracts/openzeppelin/account/Account.cairo')
    erc721_def = get_contract_def(
        'core/contracts/erc721/SuZhuTweets_Mintable_Burnable.cairo')
    erc721_holder_def = get_contract_def(
        'libs/cairo-contracts/openzeppelin/token/erc721/utils/ERC721_Holder.cairo')
    unsupported_def = get_contract_def(
        'libs/cairo-contracts/openzeppelin/security/initializable.cairo')
...
async def erc721_init(contract_defs):
    ...
    erc721_holder = await starknet.deploy(
        contract_class=erc721_holder_def,
        constructor_calldata=[]
    )
    unsupported = await starknet.deploy(
        contract_class=unsupported_def,
        constructor_calldata=[]
    )
    return (
        starknet.state,
